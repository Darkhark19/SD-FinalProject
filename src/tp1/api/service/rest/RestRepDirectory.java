...
public interface RestRepDirectory {
    ...
    @DELETE
    @Path("{" + USER_ID + "}")
    void deleteUserFiles(@HeaderParam(HEADER_VERSION)Long version,@PathParam(USER_ID) String userId, @QueryParam(PASSWORD) @DefaultValue("") String password, @QueryParam(TOKEN) String token);
